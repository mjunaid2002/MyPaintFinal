    ...
    public class LedgerController : Controller
    {
        ...
        public ActionResult LedgerSearch(AccountTitle accountTitle, string s_date, string e_date, TransactionDetail transactionDetail)
        {
            ...
            else if (headid == 2 || headid == 4)
            {
                ...
                sum0_30 = _context.Database.SqlQuery<decimal>("SELECT   isnull(SUM (cr-dr),0) as Cr FROM   TransactionDetails  where TransDate >= '" + startOfPrevious30DaysString + "'  and AccountId = " + accountTitle.AccountNo + "  AND Vtype IN ('PRINV', 'PINV') ").FirstOrDefault();
                sum31_60 = _context.Database.SqlQuery<decimal>("SELECT   isnull(SUM (cr-dr),0) as Cr FROM   TransactionDetails  where (TransDate >= '" + startOfPrevious60DaysString + "' AND TransDate < '" + startOfPrevious30DaysString + "')   and AccountId = " + accountTitle.AccountNo + " AND Vtype IN ('PRINV', 'PINV') ").FirstOrDefault();
                sum61_90 = _context.Database.SqlQuery<decimal>("SELECT   isnull(SUM (cr-dr),0) as Cr FROM   TransactionDetails  where (TransDate >= '" + startOfPrevious90DaysString + "' AND TransDate < '" + startOfPrevious60DaysString + "')  and AccountId = " + accountTitle.AccountNo + " AND Vtype IN ('PRINV', 'PINV') ").FirstOrDefault();
                ...
                sum0_30_1 = _context.Database.SqlQuery<decimal>("SELECT   isnull(SUM (cr-dr),0) as Cr FROM   TransactionDetails  where  TransDate >= '" + startOfPrevious30DaysString1 + "'  and AccountId = " + accountTitle.AccountNo + " AND Vtype IN ('PRINV', 'PINV', 'WTSINV')").FirstOrDefault();
                sum31_60_1 = _context.Database.SqlQuery<decimal>("SELECT   isnull(SUM (cr-dr),0) as Cr FROM   TransactionDetails  where (TransDate >= '" + startOfPrevious60DaysString1 + "' AND TransDate < '" + startOfPrevious30DaysString1 + "')   and AccountId = " + accountTitle.AccountNo + " AND Vtype IN ('PRINV', 'PINV') ").FirstOrDefault();
                sum61_90_1 = _context.Database.SqlQuery<decimal>("SELECT   isnull(SUM (cr-dr),0) as Cr FROM   TransactionDetails  where (TransDate >= '" + startOfPrevious90DaysString1 + "' AND TransDate < '" + startOfPrevious60DaysString1 + "')  and AccountId = " + accountTitle.AccountNo + " AND Vtype IN ('PRINV', 'PINV') ").FirstOrDefault();
