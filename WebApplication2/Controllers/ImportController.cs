...
namespace WebApplication2.Controllers
{
    ...
    public class ImportController : Controller
    {
        ...
        [HttpPost]
        public ActionResult ImportProductsRO(HttpPostedFileBase postedFile)
        {
            string filePath = string.Empty;
            if (postedFile != null)
            {
                ...
                {
                    strprd += "Update Product set ReorderLevel=" + Convert.ToDecimal(row["Reorder Level"]) + " where ProductID=" + Convert.ToDecimal(row["ProductID"]) + ";";
                    strprd += "Update ProductExtend set MaxLevelRaw=" + Convert.ToDecimal(row["Maximum Level"]) + " where ProductID=" + Convert.ToDecimal(row["ProductID"]) + ";";
                    i++;
                }
                _context.Database.ExecuteSqlCommand(strprd);
