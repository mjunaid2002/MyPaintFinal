...
namespace WebApplication1.Controllers
{
    ...
    public class FinishedProductsController : Controller
    {
        ...
        {
            var list = _context.Database.SqlQuery<Products>("select ProductName,ProductID,UnitPrice,ReorderLevel,vattax,CategoryID,[desc],Active from Product where CategoryID in (select CategoryID from Categories where RawProductCheck=0)").ToList();
            return View(list);
        }
        ...
        {
            _context.Database.ExecuteSqlCommand("insert into Product (BrandID,barcode,sp,splower,gallon,drum,CapDubbi,CapQuarter,CapGallon,CapDrum,ProductName,ProductID,itmdisc,ReorderLevel,vattax,CategoryID,[desc],Active,ReorderDubbi,ReorderQuarter,ReorderGallon,ReorderDrum) values(" + products.BrandID + "," + products.barcode + "," + products.sp + "," + products.splower + "," + products.gallon + "," + products.drum + "," + products.CapDubbi + "," + products.CapQuarter + "," + products.CapGallon + "," + products.CapDrum + ",'" + products.ProductName + "'," + products.ProductID + "," + products.itmdisc + "," + products.ReorderLevel + "," + products.vattax + "," + products.CategoryID + ",N'" + products.desc + "','" + Request["active"] + "'," + products.ReorderDubbi + "," + products.ReorderQuarter + "," + products.ReorderDubbi + "," + products.ReorderDrum + ")");
            return RedirectToAction("Index");
        }
        public ActionResult Edit(int? ID)
        {
            var products = _context.Database.SqlQuery<Products>("select BrandID,barcode,sp,splower,gallon,drum,CapDubbi,CapQuarter,CapGallon,CapDrum,ProductName,ProductID,itmdisc,ReorderLevel,vattax,CategoryID,[desc],Active,ReorderDubbi,ReorderQuarter,ReorderGallon,ReorderDrum from Product where ProductID = " + ID + "").SingleOrDefault();
            var Categories_list = _context.Database.SqlQuery<Categories>("SELECT * from Categories  where rawproductcheck!=1").ToList();
            var quality = _context.Database.SqlQuery<MianCategories>("SELECT * from MianCategories").ToList();
            var ProductVM = new ProductVM
            {
