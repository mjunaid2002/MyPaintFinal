...
namespace WebApplication2.Controllers
{
    ...
    public class SecondLevelAccountController : Controller
    {
        ...
        {
            var list = _context.Database.SqlQuery<AcMain>("SELECT Ac_second.id, Ac_second.account_no, Ac_second.a_title,(Select Top(1) a_title From Ac_main where account_no=Ac_second.ac_main and b_unit='" + Session["BusinessUnit"] + "') as FirstLevelAccount, (Select Top(1) ac_h_name From Ac_head where ac_h_id=Ac_second.head_id and b_unit='" + Session["BusinessUnit"] + "') as secondlevel FROM Ac_second where Ac_second.b_unit='" + Session["BusinessUnit"] + "'").ToList();
            return View(list);
        }
        ...
        public ActionResult Save(Ac_main Ac_main,int a_main,Ac_second ac_Second)
        {
            ...
            if (count == 0)
            {
                ...
                _context.Database.ExecuteSqlCommand("insert into ac_second values(" + ac_Second.head_id + "," + a_main + "," + final_acc + ",N'" + ac_Second.a_title + "','"+ Session["BusinessUnit"] + "')");
