...
namespace WebApplication1.Controllers
{
    ...
    public class FirstLevelController : Controller
    {
        ...
        {
            var list = _context.Database.SqlQuery<AcMain>("SELECT *,(Select ac_h_name From ac_head where ac_h_id=Ac_main.a_head) as HeadName FROM ac_main where b_unit='"+ Session["BusinessUnit"] + "'").ToList();
            return View(list);
        }
        ...
        public ActionResult Save(Ac_main Ac_main)
        {
            ...
            if (count == 0)
            {
                ...
                _context.Database.ExecuteSqlCommand("insert into Ac_main values(" + final_acc + "," + Ac_main.a_head + ",N'" + Ac_main.a_title + "'," + final_acc + ",'" + Session["BusinessUnit"] + "')");
                //var Ac_head_list = _context.Ac_head.ToList();
                //var Account_headVm = new Account_headVm
                //{
                //    Ac_head_list = Ac_head_list,
