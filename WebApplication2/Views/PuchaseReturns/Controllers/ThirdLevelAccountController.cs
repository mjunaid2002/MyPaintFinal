...
namespace WebApplication2.Controllers
{
    ...
    public class ThirdLevelAccountController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            var list = _context.Database.SqlQuery<AcMain>("SELECT dbo.AccountTitles.b_unit, dbo.AccountTitles.Id, dbo.AccountTitles.AccountNo, dbo.AccountTitles.AccountTitleName,(Select Top(1) a_title From Ac_main where account_no=AccountTitles.AccMain and b_unit='" + Bunit + "') as a_title, (Select Top(1) a_title From Ac_second where account_no=AccountTitles.SecondLevel and b_unit='" + Bunit + "') as secondlevel, (Select Top(1) ac_h_name From Ac_head where ac_h_id=AccountTitles.AccountHeadId and b_unit='" + Bunit + "') as ac_h_name FROM dbo.AccountTitles where AccountTitles.b_unit='" + Bunit + "'").ToList();
            return View(list);
        }
        ...
        public ActionResult Save(Ac_main Ac_main, int a_main, Ac_second ac_Second, AccountTitle accountTitle, int second_id)
        {
            ...
            if (count == 0)
            {
                ...
                var Bunit = Convert.ToString(Session["BusinessUnit"]);
                _context.Database.ExecuteSqlCommand("insert into accounttitles values(" + final_acc + "," + a_main + "," + accountTitle.AccountHeadId + ","+second_id+ ",N'" + accountTitle.AccountTitleName + "',N'" + accountTitle.AccountTitleName + "',0,0,'"+ Bunit + "')");
