...
namespace WebApplication1.Controllers
{
    ...
    public class SimplePurchaseController : Controller
    {
        ...
        public SimplePurchaseController()
        {
        ...
        public ActionResult Index()
        {
            ...
            var img_list = _context.Database.SqlQuery<VoucherImagesquery>("SELECT * from VoucherImages where VType = 'PINVWOCTN' and  b_unit='" + Bunit + "'").ToList();
        ...
        public ActionResult PINVWOCTNReport(int? ID, PurMaster PurMaster, PurDetail PurDetail, TransactionDetail TransactionDetail)
        {
            ...
            PurMaster.InvID = _context.Database.SqlQuery<int>("Select InvID from PurMasters where InvID =" + ID + " and InvType='PINVWOCTN' and  b_unit='" + Bunit + "'").FirstOrDefault();
            PurMaster.NetAmount = _context.Database.SqlQuery<decimal>("Select NetAmount from PurMasters where InvID =" + ID + " and InvType='PINVWOCTN' and  b_unit='" + Bunit + "'").FirstOrDefault();
            PurMaster.DiscountAmount = _context.Database.SqlQuery<decimal>("Select DiscountAmount from PurMasters where InvID =" + ID + " and InvType='PINVWOCTN' and  b_unit='" + Bunit + "'").FirstOrDefault();
            PurMaster.CargoCharges = _context.Database.SqlQuery<decimal>("Select CargoCharges from PurMasters where InvID =" + ID + " and InvType='PINVWOCTN' and  b_unit='" + Bunit + "'").FirstOrDefault();
            PurMaster.GrandTotal = _context.Database.SqlQuery<decimal>("Select GrandTotal from PurMasters where InvID =" + ID + " and InvType='PINVWOCTN' and  b_unit='" + Bunit + "'").FirstOrDefault();
            ...
            PurMaster = _context.PurMaster.SingleOrDefault(c => c.InvID == ID && c.InvType == "PINVWOCTN" && c.b_unit==Bunit);
            ...
            var p_detail = _context.Database.SqlQuery<PurDetailqueryss>("SELECT *,(Select Munit From Products where id=PurDetails.ItemID) as Munit from PurDetails where InvID=" + ID + " and InvType = 'PINVWOCTN' and  b_unit='" + Bunit + "'").ToList();
            var PurInvoiceVM = new PurInvoiceVM
            {
                sett = sett,
                p_detail = p_detail,
       ...
       public ActionResult PINVWOCTNReport2(int? ID, PurMaster PurMaster, PurDetail PurDetail, TransactionDetail TransactionDetail)
        {
            string Bunit  = Convert.ToString(Session["BusinessUnit"]);
            PurMaster.InvID = _context.Database.SqlQuery<int>("Select InvID from PurMasters where InvID =" + ID + " and InvType='PINVWOCTN' and  b_unit='" + Bunit + "'").FirstOrDefault();
            PurMaster.NetAmount = _context.Database.SqlQuery<decimal>("Select NetAmount from PurMasters where InvID =" + ID + " and InvType='PINVWOCTN' and  b_unit='" + Bunit + "'").FirstOrDefault();
        ...
        {
            var Bunit = Convert.ToString(Session["BusinessUnit"]);
            PurMaster.InvID = _context.Database.SqlQuery<int>("select ISNULL(Max(InvID),0)+1 from PurMasters where InvType = 'PINVWOCTN' and b_unit='"+ Bunit + "'").FirstOrDefault();
        ...
        [HttpPost]
        public ActionResult SearchGatepassSave(HttpPostedFileBase[] file, decimal[] length, int[] inv_id, int[] b_no, string[] exp_date, string[] srno, string[] item_name, int[] id, string[] sp, string[] qty, string[] n_total, PurMaster PurMaster, PurDetail PurDetail, TransactionDetail TransactionDetail)
        {
            ...
            for (int i = 0; i < file.Count(); i++)
            {
                ...
                _context.Database.ExecuteSqlCommand("INSERT INTO VoucherImages (Images,Tid,Vtype,b_unit) VALUES ('" + img + "'," + PurMaster.InvID + ",'PINVWOCTN','" + Session["BusinessUnit"] + "')");
            }
            for (int i = 0; i < item_name.Count(); i++)
            {
                //_context.Database.ExecuteSqlCommand("Update [GatepassDetail] set status = 1 where id = "+new_id[i]+"");
                string query = "INSERT INTO PurDetails (party,b_unit,InvId,ItemID,ItemName,SP,Qty,NetTotal,Date,Ctn,PiecesBox,InvType,g_passid) VALUES (" + PurMaster.SupplierId + ",'" + Session["BusinessUnit"] + "'," + PurMaster.InvID + "," + id[i] + ",N'" + item_name[i] + "'," + sp[i] + "," + qty[i] + "," + n_total[i] + ",'" + PurMaster.Date + "',0," + length[i] + ",'PINVWOCTN'," + inv_id[i] + ")";
                _context.Database.ExecuteSqlCommand("INSERT INTO PurDetails (BatchNumber,party,b_unit,InvId,ItemID,ItemName,SP,Qty,NetTotal,Date,Ctn,PiecesBox,InvType,g_passid) VALUES ('"+ srno[i] + "'," + PurMaster.SupplierId + ",'" + Session["BusinessUnit"] + "'," + PurMaster.InvID + "," + id[i] + ",N'" + item_name[i] + "'," + sp[i] + "," + qty[i] + "," + n_total[i] + ",'" + PurMaster.Date + "',0," + length[i] + ",'PINVWOCTN'," + inv_id[i] + ")");
            }
            ...
            _context.Database.ExecuteSqlCommand("INSERT INTO TransactionDetails (b_unit,TransId,TransDate,TransDes,AccountId,Dr,Cr,InvId,Vtype) VALUES ('" + Session["BusinessUnit"] + "'," + TransactionDetail.TransId + ",'" + PurMaster.Date + "','"+ trans_des + "'," + PurMaster.SupplierId + ",0,'" + PurMaster.GrandTotal + "'," + PurMaster.InvID + ",'PINVWOCTN')");
            _context.Database.ExecuteSqlCommand("INSERT INTO TransactionDetails (b_unit,TransId,TransDate,TransDes,AccountId,Dr,Cr,InvId,Vtype) VALUES ('" + Session["BusinessUnit"] + "'," + TransactionDetail.TransId + ",'" + PurMaster.Date + "','" + trans_des + "',1100002,'" + PurMaster.GrandTotal + "',0," + PurMaster.InvID + ",'PINVWOCTN')");
        ...
        {
            var Bunit = Convert.ToString(Session["BusinessUnit"]);
            PurMaster.InvID = _context.Database.SqlQuery<int>("select ISNULL(Max(InvID),0)+1 from PurMasters where InvID =" + ID + " and  InvType = 'PINVWOCTN' and  b_unit='" + Bunit + "'").FirstOrDefault();
            var Bunitdec = Convert.ToDecimal(Session["BusinessUnit"]);
            var Store = _context.Store.Where(z => z.b_unit == Bunit).ToList();
            var sup_list = _context.Supplier.Where(z => z.BusinessUnit == Bunitdec).ToList();
            var emp_list = _context.Employee.Where(z => z.BusinessUnit == Bunitdec).ToList();
            ...
            PurMaster = _context.PurMaster.SingleOrDefault(c => c.InvID == ID && c.InvType == "PINVWOCTN" && c.b_unit == Bunit);
            var p_detail = _context.Database.SqlQuery<PurDetailqueryss>("SELECT * from PurDetails where InvID=" + ID + " and InvType = 'PINVWOCTN' and  b_unit='" + Bunit + "'").ToList();
            var PurInvoiceVM = new PurInvoiceVM
            {
                Store = Store,
                batch_no = batch_no,
        ...
        public ActionResult Update(HttpPostedFileBase[] file, TransactionDetail TransactionDetail, PurInvoiceVM PurInvoiceVM, int[] inv_id, decimal[] length, int[] b_no, string[] exp_date, string[] item_name, int[] id, string[] sp, string[] qty, string[] n_total)
        {
            ...
            for (int i = 0; i < file.Count(); i++)
            {
                ...
                _context.Database.ExecuteSqlCommand("INSERT INTO VoucherImages (Images,Tid,Vtype,b_unit) VALUES ('" + img + "'," + PurInvoiceVM.PurMaster.InvID + ",'PINVWOCTN','" + Session["BusinessUnit"] + "')");
            }
            ...
            TransactionDetail.TransId = _context.Database.SqlQuery<int>("select ISNULL(Max(TransId),0)+1 from TransactionDetails").FirstOrDefault();
            //Transaction Data
            _context.Database.ExecuteSqlCommand("INSERT INTO TransactionDetails (b_unit,TransId,TransDate,TransDes,AccountId,Dr,Cr,InvId,Vtype) VALUES ('" + Session["BusinessUnit"] + "'," + TransactionDetail.TransId + ",'" + PurInvoiceVM.PurMaster.Date + "','"+ trans_des + "'," + PurInvoiceVM.PurMaster.SupplierId + ",0,'" + PurInvoiceVM.PurMaster.GrandTotal + "'," + PurInvoiceVM.PurMaster.InvID + ",'PINVWOCTN')");
            _context.Database.ExecuteSqlCommand("INSERT INTO TransactionDetails (b_unit,TransId,TransDate,TransDes,AccountId,Dr,Cr,InvId,Vtype) VALUES ('" + Session["BusinessUnit"] + "'," + TransactionDetail.TransId + ",'" + PurInvoiceVM.PurMaster.Date + "','" + trans_des + "',1100002,'" + PurInvoiceVM.PurMaster.GrandTotal + "',0," + PurInvoiceVM.PurMaster.InvID + ",'PINVWOCTN')");
