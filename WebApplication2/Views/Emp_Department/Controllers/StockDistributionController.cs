...
namespace WebApplication2.Controllers
{
    public class StockDistributionController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            var list = _context.Database.SqlQuery<DistributionMaster>("SELECT * from StockDistribution " + strquery).ToList();
            return View(list);
        }
        ...
        [HttpPost]
        public ActionResult Save(string[] item_name, int[] id, string[] qty, string[] ToStore, DistributionMaster distribution)
        {
            ...
            if (item_name != null)
            {
                ...
                {
                     _context.Database.ExecuteSqlCommand("INSERT INTO StockDistributionDetail(sr, pid, pname, qty, invid, FromBranchId, ToBranchId, FromStore, ToStore, date)  VALUES (" + i + "," + id[i] + ",'" + item_name[i] + "'," + qty[i] + "," + distribution.invid + ",'" + distribution.FromBranchId + "','" + distribution.ToBranchId + "','" + distribution.FromStore + "','" + ToStore[i] + "','" + DateTime.Today + "')");
               }

                _context.Database.ExecuteSqlCommand("INSERT INTO  StockDistribution(invid, date, datetime, FromStore, ToStore, note, FromBranchId, ToBranchId) VALUES (" + distribution.invid + ",'" + distribution.date + "','" + DateTime.Now + "','" + distribution.FromStore + "','0','" + distribution.note + "','" + distribution.FromBranchId + "','" + distribution.ToBranchId + "')  ");
        ...
        [HttpPost]
        public ActionResult Update(string[] item_name, int[] id, string[] qty, string[] ToStore, DistributionMaster distribution)
        {
          
           
            if (item_name != null)
            {
                _context.Database.ExecuteSqlCommand("UPDATE  StockDistribution  SET  FromStore ='"+ distribution.FromStore + "', ToStore ='0', note ='" + distribution.note + "', FromBranchId ='" + distribution.FromBranchId + "', ToBranchId ='" + distribution.ToBranchId + "'  where invid=" + distribution.invid);
