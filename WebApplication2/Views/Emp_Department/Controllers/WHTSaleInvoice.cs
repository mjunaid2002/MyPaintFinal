...
{
    [SessionTimeout]
    public class WHTSaleInvoiceController : Controller
    {
        private ApplicationDbContext _context;
        public WHTSaleInvoiceController()
        {
        ...
        public ActionResult InvoiceReport(int ID, string type)
        {
            ...
            var accno = _context.Database.SqlQuery<decimal>("SELECT isnull((Select accno From Customers where customerid=srsm.custid and  discount =0 ),0) as CustomerName  FROM srsm where OrderID =" + ID + " and title='WTSINV'").FirstOrDefault();
            var openBal = _context.Database.SqlQuery<int>("SELECT ISNULL(SUM(dr) - SUM(cr), 0) AS DR FROM AccountTitles WHERE AccountNo = " + accno + " ").FirstOrDefault();
            //var prebalance =_context.Database.SqlQuery<decimal>("SELECT ISNULL(SUM(Dr) - SUM(Cr),0) AS Dr FROM TransactionDetails WHERE (AccountId = "+ accno + ") AND (TransDate <= '"+ date.ToString("yyyy-MM-dd")+ "'  AND (InvId < " + ID + ")   )").FirstOrDefault();
            var prebalance =_context.Database.SqlQuery<decimal>("SELECT ISNULL(SUM(Dr) - SUM(Cr),0) AS Dr FROM TransactionDetails WHERE (AccountId = "+ accno + ") AND (TransDate <= '"+date.ToString("yyyy-MM-dd") + "') ").FirstOrDefault();
            
          //  var Currenttotal = _context.Database.SqlQuery<decimal>("SELECT ISNULL(SUM(Dr) - SUM(Cr),0) AS Dr  FROM TransactionDetails WHERE (AccountId = " + accno + ") AND (TransDate <= '" + date.ToString("yyyy-MM-dd") + "')  AND (InvId <= " + ID + ")  ").FirstOrDefault();
            prebalance = openBal + prebalance- grandtotal;
           var  Currenttotal = prebalance + grandtotal;
            var customer = _context.Database.SqlQuery<string>("SELECT isnull((Select name From Customers where customerid=srsm.custid  ),0) as CustomerName  FROM srsm where OrderID =" + ID + " and title='WTSINV'").FirstOrDefault();
            var customeraccno = _context.Database.SqlQuery<Decimal>("SELECT isnull((Select customerid From Customers where customerid=srsm.custid  ),0) as CustomerName  FROM srsm where OrderID =" + ID + " and title='WTSINV'").FirstOrDefault();
            var cusemail = _context.Database.SqlQuery<string>("SELECT isnull((Select email From Customers where customerid=srsm.custid  ),0) as CustomerName  FROM srsm where OrderID =" + ID + " and title='WTSINV'").FirstOrDefault();
            var cusphone = _context.Database.SqlQuery<string>("SELECT isnull((Select Phone From Customers where customerid=srsm.custid  ),0) as CustomerName  FROM srsm where OrderID =" + ID + " and title='WTSINV'").FirstOrDefault();
            var cusaddress = _context.Database.SqlQuery<string>("SELECT isnull((Select Address From Customers where customerid=srsm.custid  ),0) as CustomerName  FROM srsm where OrderID =" + ID + " and title='WTSINV'").FirstOrDefault();
            var NTN = _context.Database.SqlQuery<string>("SELECT isnull((Select NTN From Customers where customerid=srsm.custid  ),0) as CustomerName  FROM srsm where OrderID =" + ID + " and title='WTSINV'").FirstOrDefault();
            if (type == "pdf")
            {
                ReportDocument rd = new ReportDocument();
            ...
            else 
            {
                ...
                ViewData["prebalance"] = prebalance;
                ViewData["Currenttotal"] = Currenttotal;
        ...
        {
            saleReturnQuery.OrderID = _context.Database.SqlQuery<decimal>("select ISNULL(Max(OrderID),0)+1 from srsm where title='WTSINV'").FirstOrDefault();
        ...
        {
            var saleReturnQuery = _context.Database.SqlQuery<SaleReturnQuery>("select RegionId ,[OrderID] ,[empname] ,[cargoid] as inctax ,[custid] ,[date] ,[total] ,[gst] ,[discount] ,[wht] ,[cargocharges] as afterdisc ,[ntotal] ,[custname] ,[bal] ,[note] ,[pono] ,[custntn] AS invno,[custst] ,[title] ,[time],[req_status] from srsm where OrderID =" + ID + " and title='WTSINV'").SingleOrDefault();
        ...
        {
            _context.Database.ExecuteSqlCommand("Delete From TransactionDetails where InvId =" + saleReturnQuery.OrderID + " and vtype = 'WTSINV'");
            _context.Database.ExecuteSqlCommand("Delete From srsm where OrderID =" + saleReturnQuery.OrderID + " and title='WTSINV' ");
            _context.Database.ExecuteSqlCommand("Delete From srsdetail where OrderID =" + saleReturnQuery.OrderID + "  and Status='WTSINV' ");
            for (int i = 0; i < item_name.Count(); i++)
            {
                _context.Database.ExecuteSqlCommand("INSERT INTO srsdetail (gst,ltrkg,totalgst,peritmdisc,wht,Status,sr,prid,prname,sp,total,qty,dsicval,totalafterdisc,OrderID,packing,ntotal,disc_amount) " +
                    "VALUES (" + tax_val[i] + ",0," + tax_amount[i] + ",0," + wht[i] + ",'WTSINV'," + i + "," + id[i] + ",'" + item_name[i] + "'," + sp[i] + "," + n_total[i] + "," + qty[i] + "," + disc_val[i] + "," + disc_value[i] + "," + saleReturnQuery.OrderID + ",'" + packing[i] + "'," + net[i] + "," + disc_amount[i] + ")");
        ...
        public ActionResult statusrequest(int? ID)
        {
            ...
            //_context.Database.ExecuteSqlCommand("Delete From TransactionDetails where InvId =" + ID + " and Vtype = 'SINVWCTN'");
            _context.Database.ExecuteSqlCommand("UPDATE  srsm SET  req_status ='Pending'  where  title='WTSINV' AND orderid=" + ID);
            _context.Database.ExecuteSqlCommand("INSERT INTO tbl_BatchRequest(orderid, userid, username, status,batchno,department) " +
                       " VALUES('" + ID + "','" + Session["UserID"].ToString() + "','" + Session["CurrentUserName"].ToString() + "','Requested','WHTSaleInvoice','Sale')");
            return RedirectToAction("Index");
        }
        ...
        public ActionResult Updatestatus(int ID, string status, int userid)
        {
            if (Session["CurrentUserName"].ToString() == "Super Admin")
            {
                if (status == "Approved")
                {
                    _context.Database.ExecuteSqlCommand("UPDATE  srsm SET req_status = 'Complete' where  title='WTSINV' AND orderid =" + ID);

                    _context.Database.ExecuteSqlCommand("UPDATE  tbl_BatchRequest SET status = 'Approved' where department='Sale' AND batchno='WHTSaleInvoice' AND  userid='" + userid + "' AND  orderid =" + ID);
                }
                else
                {
                    _context.Database.ExecuteSqlCommand("UPDATE  srsm SET req_status = 'Request' where  title='WTSINV' AND   orderid =" + ID);
                    _context.Database.ExecuteSqlCommand("UPDATE  tbl_BatchRequest SET status = 'DisApproved' where department='Sale' AND batchno='WHTSaleInvoice' AND userid='" + userid + "' AND  orderid =" + ID);
                }
