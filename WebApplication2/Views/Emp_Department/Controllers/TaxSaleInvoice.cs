...
namespace WebApplication1.Controllers
{
    ...
    public class TaxSaleInvoiceController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            var list = _context.Database.SqlQuery<SaleReturnQuery>("SELECT OrderID,date,custname,title,total,req_status  from srsm where title='TSINV'" + strquery).ToList();
            return View(list);
        }
        ...
        {
            var saleReturnQuery = _context.Database.SqlQuery<SaleReturnQuery>("select [OrderID],[RegionId] ,[empname] ,[cargoid] as inctax ,[custid] ,[date] ,[total] ,[gst] ,[discount] ,[wht] ,[cargocharges] as afterdisc ,[ntotal] ,[custname] ,[bal] ,[note] ,[pono] ,[custntn] ,[custst] ,[title] ,[time],[req_status] from srsm where OrderID =" + ID + " and title='TSINV'").SingleOrDefault();
